            ...
            <div class="relational_information">
                ...
                <div>
                    <div  class="relational_information_content" v-for="item in HospitalDevelopment_list" :key="item.id">
                        ...
                        <div class="relational_information_content_title"  @click="welfare_activities_detail(item.id)">
                            <span >{{ item.titles }}</span>
                        </div>
                        <div class="relational_information_content_seek">
                            <div @click="welfare_activities_detail(item.id)">【详情】</div>
                    ...
                    <div class="pagination">
                        ...
                        <a   v-if="page > 1" @click="page--, getWelfareActivitiess()">上一页</a>
                        <a  v-if="page < Math.ceil(total / perPage)" @click="page++, getWelfareActivitiess()">下一页</a>
                    </div>
                </div>
            </div>
    </div>
...
const { data } = await useAsyncData( 
    () =>$fetch(`${baseURL}/hospital/getWelfareActivitiess`,
    {
        method: 'POST',
        body: {
            page: page.value,
...
const getWelfareActivitiess=async ()=>{
await $fetch(`${baseURL}/hospital/getWelfareActivitiess`, {
        method: 'POST',
        body: {
          page: page.value,
          pageSize: perPage.value,
...
const welfare_activities_detail=(id:number)=>{
    router.push({ path: `/article/welfare_activities_detail/${id}`})
}
